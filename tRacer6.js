...
window.addEventListener('load', function () {                                               // Wait for page to be fully loaded
    ...
    function timer2(finalWPM) {
        ...
        var x = setInterval(function () {
            ...
            if (count <= finalWPM) {
                ...
                localStorage.setItem("lastTimer", finalWPM);
                clearInterval(x);
            }
        }, 50);
    }
    ...
    function racePrep() {
        ...
        if (typeof (Storage) !== "undefined") {
            ...
            localStorage.setItem("lastTimer", "Pre-WPM: NA");
        }
        document.getElementById("slow").innerHTML = localStorage.getItem("lastTimer");
    }
    racePrep();
    ...
    document.onkeydown = (e) => {
        ...
        if (e.keyCode === 8 && wrongLetter != 0) {                                  // User's input is (Delete) and the wrong letter counter is greater than Zero
            ...
            if (bb > 0) {
                bb--;
                document.getElementById("L" + (curLetter + (bb))).style.color = 'black';
            } else {
                bbtog = true;
                console.log("abc " + bb)
                document.getElementById("L" + (curLetter - 1)).style.color = 'black';
